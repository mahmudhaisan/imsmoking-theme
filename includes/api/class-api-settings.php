 ...
 * @subpackage Imsmoking_Theme/Imsmoking_Theme_Api_Settings
 */
 ...
 */
if ( ! class_exists( 'Imsmoking_Theme_Api_Settings' ) ) {

	/**
	 * Imsmoking_Theme_Api_Settings
	 *
	 * @see WP_REST_Settings_Controller
	 * @package Imsmoking_Theme
	 * @since 1.0.0
	 */
	class Imsmoking_Theme_Api_Settings extends Imsmoking_Theme_Api {
		 ...
		 */
		public function run() {
			$this->type      = 'imsmoking_theme_api_settings';
		...
		public function get_item( $request ) {
			...
			$saved_options = imsmoking_theme_get_options();
		...
		public function update_item( $request ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $params, $schema ) ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The %s property has an invalid stored value, and cannot be updated to null.', 'imsmoking-theme' ), IMSMOKING_THEME_OPTION_NAME ),
					array( 'status' => 500 )
				);
			}
			...
			/* Update options */
			imsmoking_theme_update_options(
				array(
			...
			/* Update user meta */
			imsmoking_theme_update_user_meta(
				get_current_user_id(),
				array(
		...
		public function get_settings_schema() {
			$setting_properties = apply_filters(
				'imsmoking_theme_options_properties',
				array(
...
/**
 * Return instance of  Imsmoking_Theme_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return Imsmoking_Theme_Api_Settings
 */
function imsmoking_theme_api_settings() { //phpcs:ignore
	return Imsmoking_Theme_Api_Settings::get_instance();
}
imsmoking_theme_api_settings()->run();
