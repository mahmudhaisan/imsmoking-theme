...
class Imsmoking_Theme_Notice {
	...
	private function is_load_getting_started() {
		return ! imsmoking_theme_include()->get_settings( 'hide_get_started_notice' );
	}
	...
	private function is_load_review() {
		...
		$remove_review_notice_permanently         = imsmoking_theme_include()->get_user_meta( $user_id, 'remove_review_notice_permanently' );
		$remove_review_notice_temporary_date_time = imsmoking_theme_include()->get_user_meta( $user_id, 'remove_review_notice_temporary_date_time' );

		$theme_installed_date_time = imsmoking_theme_include()->get_settings( 'theme_installed_date_time' );
	...
	public function enqueue_resources() {
		...
		$unique_id = IMSMOKING_THEME_THEME_NAME . '-notice';
		...
		$deps_file = IMSMOKING_THEME_PATH . 'build/admin/notice/notice.asset.php';
		...
		wp_enqueue_script( $unique_id, IMSMOKING_THEME_URL . 'build/admin/notice/notice.js', $dependency, $version, true );
		wp_enqueue_style( $unique_id, IMSMOKING_THEME_URL . 'build/admin/notice/notice.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'imsmoking_theme_notice_localize',
			array(
				'version'             => $version,
				'nonce'               => wp_create_nonce( 'wp_rest' ),
				'rest_url'            => get_rest_url(),
				'recommended_plugins' => imsmoking_theme_get_recommended_plugins(),
				'theme_info_url'      => esc_url( menu_page_url( IMSMOKING_THEME_THEME_NAME, false ) ),
			)
		);
	...
	public function add_theme_data() {
		...
		if ( ! $theme_installed_date_time ) {
			imsmoking_theme_update_options( 'theme_installed_date_time', time() );
		}
	}
	...
	public function remove_theme_data() {
		...
		delete_option( IMSMOKING_THEME_OPTION_NAME );

		// Delete the user meta for all users.
		delete_metadata(
			'user',
			0,
			IMSMOKING_THEME_OPTION_NAME,
			'',
			true
		);
	}
...
/**
 * Return instance of  Imsmoking_Theme_Notice class
 *
 * @since 1.0.0
 *
 * @return Imsmoking_Theme_Notice
 */
function imsmoking_theme_notice() { //phpcs:ignore
	return Imsmoking_Theme_Notice::instance();
}
imsmoking_theme_notice()->run();
