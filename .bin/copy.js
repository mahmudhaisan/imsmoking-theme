...
const filterFunction = ( file ) => {
	...
	const isExcluded =
		...
		excludedFiles.includes( fileExtension );
	if ( isExcluded ) {
	...
	if ( error ) {
		...
		ncp( source, destination, { filter: filterFunction }, ( error ) => {
			if ( error ) {
				...
				console.log(
					"The theme is built in the `deploy` folder for production. You can upload it to your WordPress site or to WordPress.org. Don't forget to check the files and folders, and test it thoroughly."
				);
			}
		} );
	}
