		...
		document.addEventListener( 'click', ( event ) => {
			if (
				event.target.classList.contains(
					'imsmoking-handler-theme-gsn-close'
				)
			) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				event.target.classList.contains(
					'imsmoking-handler-theme-rn-permanent-close'
				)
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				event.target.classList.contains(
					'imsmoking-handler-theme-rn-temporary-close'
				)
			) {
				this.modifySettings(
				...
				event.target.classList.contains(
					'imsmoking-handler-theme-install-plugins'
				)
			) {
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'imsmoking-handler-theme-review-notice'
			);
